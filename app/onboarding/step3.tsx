// Step 3 of 5 — Give: what can you offer to other creatives?
...
export default function Step3() {
  ...
  const [tags, setTags] = useState<string[]>(draft.give ?? []);

  const canContinue = tags.length >= 2;

  function handleContinue() {
    updateDraft({ give: tags });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.heading}>What do you give?</Text>
        <Text style={styles.subheading}>
          Skills, time, knowledge — what can you bring to a collaboration? Add at least 2.
        </Text>

        <TagInput
          tags={tags}
          onChange={setTags}
          placeholder="e.g. color grading, script feedback, sound design…"
        />

        <View style={styles.examples}>
          <Text style={styles.examplesLabel}>Ideas</Text>
          <View style={styles.examplePills}>
            {['directing', 'editing', 'DP work', 'music composition', 'color grading',
              'sound design', 'photography', 'animation', 'script notes', 'producing'].map((ex) => (
              <Pressable
                key={ex}
      ...
      <View style={styles.footer}>
        <Text style={styles.counter}>{tags.length} added · need at least 2</Text>
        <Pressable
